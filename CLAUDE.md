...
```text
ÚLTIMO_INCREMENTO: Pantalla "Todas las tareas" añadida como vista predeterminada de Tareas. Reúne únicamente tareas activas no eliminadas (excluye completed/canceled), ordenadas de la captura más reciente a la más antigua, y muestra un solo estado vigente por tarea. Las completadas aparecen exclusivamente en la pestaña "Completadas". Para status='planned', el identificador se deriva de execution_date mediante BUCKET_LABEL (Vencida / En progreso / Próxima acción / Planificada); los demás usan TASK_STATUS_LABEL. Desde "Todas" se conservan Procesar para Inbox y Planificar/Reprogramar cuando corresponda. La ruta muestra un estado de error si falla la consulta. Producción: https://mossie-two.vercel.app.
FASE_TÉCNICA_ACTUAL: Fase 1 en progreso (captura 4 tipos + Inbox + procesamiento GTD + planificación núcleo con execution_date derivado).
ETAPA_FUNCIONAL_ACTUAL: Capturar→Procesar→Planificar→Hoy y consulta global de tareas funcionando. Falta: en_espera (motivo obligatorio) desde el Centro de Mando; regla "3 días en Inbox → Algún día"; Delegadas (responsable/seguimiento) y recurrencia; vistas Ideas/Calendario/Finanzas.
SIGUIENTE_ACCIÓN: CUT 3 — en_espera con motivo obligatorio desde el Centro de Mando.
BLOQUEOS: ninguno
VERIFICADO_CON: CUT 2 y corrección de semántica activa/completada pasan Prettier, TypeScript, ESLint y next build con los binarios fijados en node_modules. "Todas" filtra completed/canceled y su contador usa únicamente tareas activas; "Completadas" conserva el filtro status='completed'. La ruta local /tareas redirige correctamente a /login en un navegador sin sesión; la comprobación visual con datos reales requiere una sesión autenticada. Verificación previa en vivo (localhost:3000, Supabase bnxdvrjzkdoqbnfalbws, migración 0007 aplicada por la usuaria): planificar "Hoy" movió la tarea de Por planificar(5→4) a Planificadas(1) con chip "En progreso · 12 sept" y apareció en Centro de Mando→Hoy; planificar "Mañana" apareció en Preparar mañana con chip "Próxima acción · 13 sept". Sin sección Vencidas cuando no hay. Semana inicia domingo (Mañana = dom 13 sept). Migraciones 0002–0007 aplicadas.
```
