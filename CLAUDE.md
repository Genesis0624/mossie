...
```text
ÚLTIMO_INCREMENTO: Alineación de Fase 1 al Flujo GTD v2.0. (1) Navegación del módulo Tareas: abre por defecto en Inbox; orden Inbox·Por planificar·Planificadas·Delegadas·En espera·Bloqueadas·Algún día·Todas·Completadas ("Todas" penúltima, antes de Completadas). (2) Centro de Mando: indicador de Inbox con conteo y acceso directo a Procesar (neutro cuando está vacío, destacado con pendientes). (3) Separación En espera vs Bloqueadas (Flujo v2.0 §17-§18): "En espera" (status='waiting') = fecha de revisión OBLIGATORIA (review_at) + motivo OPCIONAL, limpia execution_date; "Bloqueadas" (status='blocked', nuevo) = requisito de desbloqueo OBLIGATORIO (block_requirement), no permite Completar (candado), Desbloquear→Por planificar. Ambas y "Bloquear/Poner en espera" viven en el menú de cualquier tarea activa procesada; Reanudar/Desbloquear→to_plan. Migración 0009 añade el estado 'blocked' (recrea el CHECK) + columnas review_at y block_requirement. Producción: https://mossie-two.vercel.app.
FASE_TÉCNICA_ACTUAL: Fase 1 en progreso, alineándose al Flujo v2.0 (captura 4 tipos + Inbox + procesamiento GTD + planificación núcleo + bandejas En espera y Bloqueadas).
ETAPA_FUNCIONAL_ACTUAL: Capturar→Procesar→Planificar→Hoy, consulta global (Todas), En espera y Bloqueadas funcionando. Falta (Flujo v2.0): campos de planificación (duración + regla de 3 min, energía requerida y efecto energético, contexto múltiple, prioridad); reprogramación con contador (reschedule_count) e historial (task_history, incl. guardar execution_date previa al pausar); Delegadas (responsable/seguimiento vinculado) y recurrencia (reglas + ocurrencias); Inbox de ideas; Calendario/Finanzas.
SIGUIENTE_ACCIÓN: CUT de campos de planificación (Flujo v2.0 §12): duración + regla de 3 min, energía requerida y efecto energético, contexto múltiple (task_contexts) y prioridad.
BLOQUEOS: ninguno (migraciones 0008 y 0009 aplicadas por la usuaria).
VERIFICADO_CON: Prettier, TypeScript, ESLint y next build (Next 16.3.5) en verde. Verificación EN VIVO (localhost:3000, sesión de la usuaria, migraciones 0002–0009 aplicadas): Tareas abre en Inbox; orden Inbox·Por planificar·Planificadas·Delegadas·En espera·Bloqueadas·Algún día·Todas·Completadas (Todas penúltima). Indicador de Inbox en Home muestra "4 por procesar" destacado. Poner en espera desde una Planificada: pidió fecha de revisión (default 1 semana) + motivo opcional; la tarea salió de Planificadas(2→1), apareció en En espera con chip "En espera: <motivo>" y botón Reanudar, sin fecha de ejecución. Bloquear otra Planificada: requisito obligatorio (botón atenuado sin texto); la tarea salió de Planificadas, apareció en Bloqueadas con candado (sin control de completar) y chip "Bloqueada: <requisito>"; Desbloquear la devolvió a Por planificar(1→2). completeTask usa .neq('status','blocked') como refuerzo en servidor.
```
