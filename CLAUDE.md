...
```text
ÚLTIMO_INCREMENTO: Procesamiento en orden (sin migración) — botón "Procesar en orden" en el Inbox que abre el modal en la 1ª tarea y, al Guardar, avanza automáticamente a la siguiente ("Guardar y siguiente", con progreso "N de M") hasta vaciar el Inbox. ProcessSheet ahora acepta onProcessed/progress; TareasTabs maneja la cola de ids. Antes: procesamiento con Tipo/Pilar/Fecha límite, Atender hoy, checklist de pasos, recurrente. Producción: https://mossie-two.vercel.app.
...
SIGUIENTE_ACCIÓN: PLANIFICACIÓN. Reglas definidas por la usuaria: (1) semana inicia DOMINGO; (2) el día corta a medianoche (hoy=día actual en zona Santo Domingo; mañana tras 00:00); (3) una tarea de Por planificar pasa a PLANIFICADA solo cuando tiene fecha de ejecución (execution_date); sin fecha sigue en Por planificar; con fecha (aunque falten otros campos) ya está planificada y editable luego; (4) 3 días en Inbox sin procesar → Algún día (requiere cron o cálculo on-read); (5) estados generales a soportar: inbox, por_planificar(to_plan), en_progreso (Centro de Mando/hoy), proxima_accion (día siguiente), en_espera (con motivo obligatorio), completada, cancelada (+ delegated, someday). Falta añadir execution_date y estados next_action; "planned" actual se remapeará a en_progreso/proxima_accion según fecha.
BLOQUEOS: ninguno
VERIFICADO_CON: pnpm typecheck, lint y build OK; "Procesar en orden" con 4 tareas → avanzó 1→2→3→4 guardando cada una y cerró al final dejando el Inbox vacío. Contra Supabase (proyecto bnxdvrjzkdoqbnfalbws). Migraciones 0002–0006 aplicadas (este incremento no requiere migración).
```
