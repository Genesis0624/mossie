...
```text
ÚLTIMO_INCREMENTO: Planificación (núcleo). Migración 0007 añade tasks.execution_date (date) + índice parcial. Nueva acción planTask(id, fecha) → status='planned' + execution_date (limpia attend_today). Drawer PlanSheet (Hoy / Mañana / Otra fecha) desde "Por planificar" y "Planificadas" (esta última reprograma). CLAVE: los estados en_progreso/proxima_accion/vencida/planificada NO se guardan; se DERIVAN de execution_date vs hoy en zona Santo Domingo (src/features/tasks/dates.ts: sdDateString/addDays/planBucket/BUCKET_LABEL), así el corte a medianoche ocurre solo sin cron. Centro de Mando ahora muestra Hoy (=hoy), Vencidas (<hoy, solo si hay), Preparar mañana (=mañana) y Tareas exprés. Nueva pestaña "Planificadas" en Tareas; TaskRow acepta canPlan y muestra chip "Estado · fecha". Producción: https://mossie-two.vercel.app.
...
ETAPA_FUNCIONAL_ACTUAL: Capturar→Procesar→Planificar→Hoy funcionando de extremo a extremo. Falta: pantalla "Todas las tareas con su identificador" (una sola lista, todo estado); en_espera (motivo obligatorio) desde el Centro de Mando; regla "3 días en Inbox → Algún día"; Delegadas (responsable/seguimiento) y recurrencia; vistas Ideas/Calendario/Finanzas.
SIGUIENTE_ACCIÓN: CUT 2 — pantalla "Todas las tareas": una sola lista con TODA tarea sin importar estado, cada una con su identificador/estado (usar BUCKET_LABEL para planificadas + label por status para el resto). Después: en_espera con motivo obligatorio.
BLOQUEOS: ninguno
VERIFICADO_CON: pnpm format/typecheck/lint/build OK. En vivo (localhost:3000, Supabase bnxdvrjzkdoqbnfalbws, migración 0007 aplicada por la usuaria): planificar "Hoy" movió la tarea de Por planificar(5→4) a Planificadas(1) con chip "En progreso · 12 sept" y apareció en Centro de Mando→Hoy; planificar "Mañana" apareció en Preparar mañana con chip "Próxima acción · 13 sept". Sin sección Vencidas cuando no hay. Semana inicia domingo (Mañana = dom 13 sept). Migraciones 0002–0007 aplicadas.
```
