...
```text
ÚLTIMO_INCREMENTO: Procesamiento — rutas afinadas + varios pasos + recurrencia. Migración 0006 añade tasks.attend_today (bool), is_recurring (bool) y checklist (jsonb [{text,done}]). "Atender ahora" ahora manda a Por planificar (to_plan) pero con attend_today=true → indicador "Atender hoy" (arcilla) en la tarjeta. "¿Tiene varios pasos?" abre un editor de subtareas (texto + eliminar + añadir) en el modal; en la tarjeta se muestran colapsadas (N/M pasos) y se despliegan al pasar el mouse o tocar, con checkbox que persiste (updateTaskChecklist). "¿Es recurrente?" solo marca única/recurrente (ícono ↻; la frecuencia se define al planificar). Delegar sigue yendo a Delegadas (modal de responsable pendiente). En espera queda fuera del procesamiento. Antes: Tipo/Pilar/Fecha límite en el modal; matriz Eisenhower; /tareas con pestañas y filtros. Producción: https://mossie-two.vercel.app.
...
SIGUIENTE_ACCIÓN: Delegadas (estado de delegación + campos responsable/seguimiento en su pantalla), o planificación real (execution_date, Hoy/Preparar mañana + En espera con motivo), o recurrencia (constructor de frecuencia + ocurrencias).
BLOQUEOS: ninguno
VERIFICADO_CON: pnpm typecheck, lint y build OK; procesé una tarea como Atender ahora con 2 pasos y recurrente → quedó en Por planificar con "Atender hoy", ícono ↻ y checklist 0/2 que al marcar pasó a 1/2 y persistió. Contra Supabase (proyecto bnxdvrjzkdoqbnfalbws). Migraciones 0002–0006 aplicadas.
```
