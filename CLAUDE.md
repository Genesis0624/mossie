...
VERIFICADO_CON: Prettier, TypeScript, ESLint y next build (Next 16.3.5) en verde. Verificación EN VIVO (localhost:3000, sesión de la usuaria, migraciones 0002–0011 aplicadas): Tareas abre en Inbox con el orden del doc; indicador de Inbox en Home. En espera y Bloqueadas OK (chip, Reanudar/Desbloquear→Por planificar, candado impide completar). Planificación: fecha + hora + duración (regla de 3 min al elegir ≤3) + energía + efecto + prioridad se guardan y muestran en la tarjeta y en Centro de Mando→Hoy. Contexto múltiple: seleccionar Trabajo + Computadora se guardó en task_contexts y la tarjeta muestra "Contexto: Trabajo, Computadora"; combinaciones y RLS OK. completeTask usa .neq('status','blocked') como refuerzo en servidor.
```
