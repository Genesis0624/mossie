...
```text
ÚLTIMO_INCREMENTO: Pantalla "Todas las tareas" añadida como vista predeterminada de Tareas. Reúne toda tarea no eliminada, ordenada de la captura más reciente a la más antigua, y muestra su estado legible. Para status='planned', el identificador se deriva de execution_date mediante BUCKET_LABEL (Vencida / En progreso / Próxima acción / Planificada); los demás usan TASK_STATUS_LABEL. Desde "Todas" se conservan Procesar para Inbox y Planificar/Reprogramar cuando corresponda. Las completadas/canceladas ya no ofrecen volver a completar. La ruta muestra un estado de error si falla la consulta. Producción pendiente de este incremento: https://mossie-two.vercel.app.
